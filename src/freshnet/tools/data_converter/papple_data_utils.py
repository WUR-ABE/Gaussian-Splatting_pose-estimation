...
class PappleInstance(object):
    ...
    def __init__(self, line):
        ...
        self.centroid = np.array([data[5], data[6], data[7]]) # If increase for KITTI, * 10.0 
        self.width = data[8]  / 2 # If increase for KITTI, * 10.0 
        self.length = data[9]  / 2 # If increase for KITTI, * 10.0 
        self.height = data[10]  / 2 # If increase for KITTI, * 10.0 
        self.size = np.array([float(data[9]), float(data[8]), float(data[10])]) # If increase for KITTI, * 10.0 
...
class PAPPLEData(object):
    ...
    def __init__(self, root_path, save_path, split_dir="papple_trainval", split='train', use_v1=False):
        self.root_dir = root_path
        self.save_path = save_path
        self.split = split
        self.split_dir = osp.join(root_path, split_dir)
        self.classes = [
            'apple'
        ]
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            mmcv.mkdir_or_exist(osp.join(self.root_dir, self.save_path, 'points'))
            pc_upright_depth_subsampled.tofile(
                osp.join(self.root_dir, self.save_path, 'points', f'{sample_idx:06d}.bin'))

            info['pts_path'] = osp.join(self.save_path, 'points', f'{sample_idx:06d}.bin')
            img_path = osp.join(self.split_dir, 'image', f'{sample_idx:06d}.jpg')
            image_info = {
            ...
            if has_label:
                ...
                if annotations['gt_num'] != 0:
                    ...
                    annotations['location'] = np.concatenate([
                        (obj.centroid * 10.0).reshape(1, 3) for obj in obj_list # If increase for KITTI, remove * 10.0
                        if obj.classname in self.cat2label.keys()
                    ],
                                                             axis=0)
                    ...
                    ## Multiplication might be required because of pretraining on KITTI, but RPY should be in radians
                    ## Can be moved to PappleInstance class to exclude orientation (KITTI)
