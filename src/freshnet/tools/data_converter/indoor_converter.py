...
def create_indoor_info_file(data_path,
    ...
    if save_path is not None:
        save_path = save_path[1:] # Strips the leading slash
    ...
    if save_path and not os.path.exists(os.path.join(data_path, save_path)):
        os.makedirs(os.path.join(data_path, save_path))
    save_path = data_path if save_path is None else save_path
    assert os.path.exists(os.path.join(data_path, save_path))

    # generate infos for both detection and segmentation task
    if pkl_prefix in ['sunrgbd', 'scannet', 'papple']:
        train_filename = os.path.join(os.path.join(data_path, save_path),
                                      f'{pkl_prefix}_infos_train.pkl')
        val_filename = os.path.join(os.path.join(data_path, save_path), f'{pkl_prefix}_infos_val.pkl')
        if pkl_prefix == 'sunrgbd' or pkl_prefix == 'sunrgbd_small':
            ...
            train_dataset = PAPPLEData(
                root_path=data_path, save_path=save_path, split_dir=split_dir, split='train')
            print("Loading val dataset")
            val_dataset = PAPPLEData(
                root_path=data_path, save_path=save_path, split_dir=split_dir, split='val')
            print("Loading test dataset")
            test_dataset = PAPPLEData(
                root_path=data_path, save_path=save_path, split_dir=split_dir, split='test')
            test_filename = os.path.join(os.path.join(data_path, save_path),
