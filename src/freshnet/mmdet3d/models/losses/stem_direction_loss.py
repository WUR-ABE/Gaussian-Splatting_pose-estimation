...
def stem_axis_vector_dot_product(pred_rpy, target_rpy):
    ...
    # if pred_rpy.max() >= torch.pi:
    #     print(f"Predictions should be smaller than pi, but are {pred_rpy.max()}")
    # if pred_rpy.min() <= -torch.pi:
    #     print(f"Predictions should be greater than -pi, but are {pred_rpy.min()}")
